...
const generateDummyData = () => {
  const data = [];
  for (let i = 0; i < 2; i++) {
    const receivedDb = Math.floor(Math.random() * 500) + 100; // 100-600
    const successfulCalls = Math.floor(receivedDb * (0.3 + Math.random() * 0.4)); // 30-70%
    const failedCalls = Math.floor(receivedDb * (0.1 + Math.random() * 0.2)); // 10-30%
    const unperformed = receivedDb - successfulCalls - failedCalls;
    const progressRate = ((successfulCalls / receivedDb) * 100).toFixed(1);
    const successfulRegistrations = Math.floor(successfulCalls * (0.2 + Math.random() * 0.3)); // 20-50%
    const registrationRate = ((successfulRegistrations / successfulCalls) * 100).toFixed(1);
    const totalRank = Math.floor(Math.random() * 50) + 50; // 50-100
    const myRank = Math.floor(Math.random() * totalRank) + 1; // 1-totalRank
...
const statusData = ref(generateDummyData());

// 내 OBTM 현황 페이징 관련
const statusTotalCount = ref(0);
const statusCurrentPage = ref(1);
const statusItemsPerPage = ref(10);

const statusTotalPages = computed(() => {
  return Math.ceil(statusTotalCount.value / statusItemsPerPage.value);
});
...
const generateListData = () => {
  ...
  for (let i = 0; i < 5; i++) {
    data.push({
      ...
      callSuccess:
        callSuccessOptions[Math.floor(Math.random() * callSuccessOptions.length)],
      previousResult:
        previousResultOptions[
          Math.floor(Math.random() * previousResultOptions.length)
        ],
...
const listData = ref(generateListData());
...
const listTotalPages = computed(() => {
  return Math.ceil(listTotalCount.value / listItemsPerPage.value);
});
...
// 총건수 초기화
statusTotalCount.value = statusData.value.length;
listTotalCount.value = listData.value.length;
</script>
